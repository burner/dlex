...
final public class DFA { 
	...
	private static immutable STATES = 500;

	// The code for "no target state" in the transition table.
	public static immutable NO_TARGET = -1;
	...
	Action[Action] usedActions = new Action[Action];

	// True iff this DFA contains general lookahead
	bool lookaheadUsed;

	public this(int numEntryStates, int numInp, int numLexStates) {
		this.numInput = numInp; 

		int statesNeeded = max(numEntryStates, STATES);

		this.table = new int[this.statesNeeded][this.numInput];
		this.action = new Action[this.statesNeeded];
		this.isFinal = new bool[this.statesNeeded];
	...
	private void ensureStateCapacity(int newNumStates) {
		...
		bool[] newFinal = new bool[newLength];
		bool[] newPushback = new bool[newLength];
		Action[] newAction = new Action[newLength];
		int[][] newTable = new int[newLength][numInput];

		arrayCopy(isFinal,0,newFinal,0,numStates);
		arrayCopy(action,0,newAction,0,numStates);
		arrayCopy(table,0,newTable,0,oldLength);

		for(uint i = oldLength; i < newLength; i++) {
	...
	public override string toString() {
		...
		for(int i=0; i < numStates; i++) {
			...
			for(char j=0; j < numInput; j++) {
				if(table[i][j] >= 0) 
					result.pushBack("  with " ~ conv!(int,string)(j) ~ " in " 
						~ table[i][j] ~ "\n");	
			}
		}
	...
	public void checkActions(LexScan scanner, LexParse parser) {
		...
		Vector!(Action) l = scanner.actions.elements();

		foreach(Action it; l) {
			if(!it.equals(usedActions.get(it)) && !eofActions.isEOFAction(it)) {
				Out.warning(scanner.file, ErrorMessages.NEVER_MATCH, it.priority-1, -1);
			}
		}
	}
