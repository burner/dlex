...
final public class Macros {
	...
	public bool insert(string name, RegExp definition) {
		...
		if (Options.DEBUG) 
			Out.debugPrint("inserting macro "~name~" with definition :"~Out.NL~definition); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public bool isUsed(string name) {
		//return ((Boolean)used.get(name)).boolValue();
		return used[name];
	}
