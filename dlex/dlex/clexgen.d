...
class CLexGen {
	...
	void generate() {
		...
		if (m_spec.m_verbose) {
			wirteln("Processing first section -- user code.");
		}
		userCode();
	...
	private void userDeclare() {
		...
		while (false == m_input.getLine()) {
			...
			if (CUtility.OLD_DEBUG) {
				writeln("Line number " ~ m_input.m_line_number ~ ":");
				writeln(new String(m_input.m_line, 0, m_input.m_line_read));
			}
		}
	}
	...
	private string interp_int(int i) {
		...
		case conv!(char,int)('\f'):
			return "\\f";

		case conv!(char,int)('\r'):
			return "\\r";
